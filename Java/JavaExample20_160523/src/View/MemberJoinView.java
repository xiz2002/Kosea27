...
public class MemberJoinView extends Frame {
	...
	// 화면 컴포넌트(멤버 필드) 구성 시작
	JLabel jlbMsg;
	Label lblSearch, lblMsg, lblId, lblPw, lblPwVal, lblName, lblAddr, lblAddrDe, lblPhone;
	TextField tfSearch, tfId, tfPw, tfPwVal, tfName, tfAddr, tfAddrDe, tfPhone;
	Panel pnlSearch, pnlResult, pnlId, pnlPw, pnlPwVal, pnlName, pnlAddr, pnlAddrDe, pnlPhone, pnlButton, pnlMsgLabel, pnlMsgButton;
	Button btnSearch, btnUpdate, btnReset, btnMsg;
	Dialog diaMsg;
	...
	private void showUpdateComponents(final MemberVO member) {
		...
		lblPwVal = new Label("Password Validation : ");
		...
		btnUpdate.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				Pattern p = Pattern.compile("/010-\\d{4}-\\d{1,4}/");
				Matcher m = p.matcher(tfPhone.getText().toString().trim());
				if (tfPw.getText().hashCode() != tfPwVal.getText().hashCode()) {
					printMsgInfo(tfPw,tfPwVal,"<html><b>신규 패스워드가 일치 하지 않습니다!</b><br><b>비워둘시 기존패스워드를 적용합니다.</b></html>");
				} else if (!m.matches()) {
					printMsgInfo(tfPhone,"<html><b>폰 넘버 형식이 다릅니다.</b><br><b>Ex)010-1234-1234</b></html>");
				} else {
					dao = MemberDAOServiceImpl.getInstance();
					try {
						if (!tfPw.getText().isEmpty()) {
							member.setPw(tfPw.getText().toString().trim());
						}
						member.setAddress(tfAddr.getText().toString().trim() + "*" + tfAddrDe.getText().toString().trim());

						member.setPhone(tfPhone.getText().toString().trim());
						dao.updateMember(member);
						printMsgInfo("변경완료되었습니다.");
		...
		pnlPwVal.setLayout(new FlowLayout(FlowLayout.LEFT, 20, 10));
		...
		pnlPwVal.add(lblPwVal);
		pnlPwVal.add(tfPwVal);
